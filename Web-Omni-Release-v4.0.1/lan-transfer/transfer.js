...
(function () {
  ...
  let effectiveRoomCode = roomCode; // 当 ID 冲突时会变化
  ...
  // 多个 PeerJS 信令服务器（容错）
  // 修复 #13: 移除已停服的 heroku，依赖默认 0.peerjs.com 主备
  const PEER_SERVERS = [
  ...
  function createPeer(serverIdx) {
    ...
    peer.on('open', (id) => {
      ...
      renderQRCode(effectiveRoomCode, $('qrTarget'));
      $('roomCodeDisplay').textContent = effectiveRoomCode;
    });
    ...
    peer.on('error', (err) => {
      ...
      if (err.type === 'unavailable-id') {
        // 修复 #14: ID 被占用时，重新生成全新房间码并同步到 UI 与 QR
        const newRoomCode = generateRoomCode();
        // 把外层 const roomCode 改为可写值（在函数闭包中重新赋值）
        try {
          // ES const 不能重新赋值，但我们用闭包变量 effectiveRoomCode 同步显示
          effectiveRoomCode = newRoomCode;
        } catch (_) {}
        const newPeerId = 'wo-' + newRoomCode.toLowerCase();
        peer = new Peer(newPeerId, opts);
        peer.on('open', () => {
          renderQRCode(newRoomCode, $('qrTarget'));
  ...
  $('copyCode').addEventListener('click', () => {
    navigator.clipboard.writeText(effectiveRoomCode).then(() => {
  ...
  $('downloadMobile').addEventListener('click', () => {
    ...
    generateMobilePage(pid, effectiveRoomCode);
  });
