...
(function() {
  ...
  function clearPageCookies() {
    ...
    if (window.webOmniShowToast) window.webOmniShowToast(`已清除 ${cleared} 个 Cookie（仅可读，HttpOnly Cookie 浏览器禁止 JS 访问）`, 'success');
  }
  ...
  function fingerprintProtect() {
    ...
    // Canvas 指纹噪声注入（修复：在原画布上做克隆，避免污染网站正常渲染）
    try {
      ...
      HTMLCanvasElement.prototype.toDataURL = function(type, quality) {
        ...
        try {
          ...
          // 仅扰动一个 10x10 角落区域，不需读全画布（跨域时 getImageData 会抛错）
          const sw = Math.min(w, 10), sh = Math.min(h, 10);
          try {
            const imageData = cctx.getImageData(0, 0, sw, sh);
            for (let i = 0; i < imageData.data.length; i += 4) {
              imageData.data[i]   ^= (Math.random() > 0.5 ? 1 : 0);
              imageData.data[i+1] ^= (Math.random() > 0.5 ? 1 : 0);
            }
            cctx.putImageData(imageData, 0, 0);
          } catch (e) { /* tainted canvas; fall back to original */ }
          // 用克隆的 canvas 调用原始 toDataURL
      ...
      HTMLCanvasElement.prototype.toBlob = function(callback, type, quality) {
        const w = this.width, h = this.height;
        if (!w || !h) return origToBlob.apply(this, arguments);
        try {
          const clone = document.createElement('canvas');
          clone.width = w; clone.height = h;
          const cctx = clone.getContext('2d');
    ...
    // Navigator 属性伪装（加 configurable: true 防止某些站点已 redefine 后再次 set 抛错）
    try {
