...
(function() {
  ...
  // ===== 一键填充（出于安全考虑，content script 中无主密码无法解密；引导到 Vault 页面） =====
  async function vaultAutoFill() {
    ...
    if (!stored.woVaultSalt || !stored.woVaultEntries) {
      if (window.webOmniShowToast) window.webOmniShowToast("尚未创建密码库，正在打开管理器...", "warn");
      window.open(chrome.runtime.getURL("vault/index.html"), "_blank");
      return;
    }
    // 安全限制：解密需要主密码 + 安全码，content script 不持有任何敏感凭据
    if (window.webOmniShowToast) {
      window.webOmniShowToast("出于安全考虑，请在密码管理器中复制凭据再粘贴", "info", 3500);
    }
    window.open(chrome.runtime.getURL("vault/index.html"), "_blank");
  }
