...
(function() {
  ...
  function onClick(e) {
    ...
    // 修复 #8: 用 display:none 而不是 .remove()，与 applyStoredRules 行为一致，便于撤销与刷新恢复
    const prevDisplay = target.style.display;
    removedElements.push({ element: target, prevDisplay });
  ...
  function undoRemove() {
    ...
    if (last.element) {
      // 还原 display
      if (last.prevDisplay) last.element.style.setProperty("display", last.prevDisplay);
      else last.element.style.removeProperty("display");
      // 同时从规则库移除最近一条对应规则
      try {
        chrome.storage.local.get(["dictatorRules"], (result) => {
          const rules = result.dictatorRules || {};
          const domain = location.hostname;
          if (rules[domain] && rules[domain].length) {
            rules[domain].pop();
  ...
  // 页面加载时自动应用已保存的规则
  // 修复 #7：缓存当前域规则到内存，避免 MutationObserver 触发时反复 chrome.storage IO
  let cachedRulesForDomain = [];

  function applyRules() {
    if (!cachedRulesForDomain.length) return;
    let applied = 0;
    cachedRulesForDomain.forEach((rule) => {
      try {
  ...
  function refreshCachedRules() {
    ...
    chrome.storage.local.get(["dictatorRules"], (result) => {
      const rules = result.dictatorRules || {};
      cachedRulesForDomain = rules[domain] || [];
      const applied = applyRules();
      if (applied > 0) console.log(`【Web-Omni】自动隐藏了 ${applied} 个已标记元素`);
    });
  }

  setTimeout(refreshCachedRules, 800);

  // 监听规则变化（用户在规则库面板修改后无需刷新）
  try {
    chrome.storage.onChanged.addListener((changes, area) => {
      if (area === "local" && changes.dictatorRules) {
        const domain = location.hostname;
        const newRules = changes.dictatorRules.newValue || {};
        cachedRulesForDomain = newRules[domain] || [];
        applyRules();
      }
    });
  } catch (e) { /* ignore */ }

  // MutationObserver: 限频 + 仅使用缓存，不再每次都打 storage
