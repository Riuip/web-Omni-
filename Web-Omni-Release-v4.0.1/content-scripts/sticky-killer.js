...
(function() {
  ...
  function restoreAll() {
    killed.forEach(item => {
      // 修复 #20: kill 时用了 setProperty(..., 'important')，需用 setProperty 还原才有效
      if (item.originalPosition) item.el.style.setProperty('position', item.originalPosition);
      else item.el.style.removeProperty('position');
      if (item.originalDisplay) item.el.style.setProperty('display', item.originalDisplay);
      else item.el.style.removeProperty('display');
      if (item.originalZIndex) item.el.style.setProperty('z-index', item.originalZIndex);
      else item.el.style.removeProperty('z-index');
    });
    killed = [];
    isActive = false;
    hideBar();
