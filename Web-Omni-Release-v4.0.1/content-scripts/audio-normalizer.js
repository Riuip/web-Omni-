...
(function() {
  ...
  function toggleNormalize() {
    ...
    if (isActive) {
      ...
      stopObserving();
      // 已绑定的 MediaElementSource 一旦创建就无法解绑，
      // 关闭时改为：把 compressor 设为 bypass，并把 gain 复位为 1.0，最大限度还原原音
      if (compressor) applyPreset('bypass');
      if (gainNode) gainNode.gain.setValueAtTime(1.0, ctx.currentTime);
      if (window.webOmniShowToast) {
        window.webOmniShowToast('音频均衡已关闭（音频信号已直通）', 'info');
      }
    }
    updatePanel();
  }
