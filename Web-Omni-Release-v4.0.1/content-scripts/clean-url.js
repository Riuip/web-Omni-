...
(function() {
  ...
  // 通用追踪参数黑名单 (使用 Set 去重 + O(1) 查询)
  // 修复 #17: 之前数组里有 utm_source/medium/campaign/term/utm_id 等重复
  const TRACKING_PARAMS_LIST = [
    ...
    'wechatShareId', 'from_scene',
    'channel', 'subchannel', 'chn',
    'callback', 'jsonp', '_', 'timestamp', 'nonce',
    'pos', 'srcid', 'biz_id', 'pgc_id',
  ];
  const TRACKING_PARAMS = new Set(TRACKING_PARAMS_LIST.map(s => s.toLowerCase()));
  ...
  function cleanUrl(rawUrl) {
    ...
    url.searchParams.forEach((_, key) => {
      ...
      // 精确匹配 (Set: O(1))
      if (TRACKING_PARAMS.has(k)) { keysToRemove.push(key); return; }
      // 前缀匹配
