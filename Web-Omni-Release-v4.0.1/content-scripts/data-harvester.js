...
(function() {
  ...
  function dumpJsGlobals() {
    // 修复 #18: window.frames[0] 通常 undefined，会让 defaultKeys 几乎为空，把全部 window 都列出来。
    // 改为：仅使用一个隐藏 iframe 作为 baseline。
    ...
    try {
      ...
      // 兜底：使用一组已知的浏览器内置全局名作为基线
      const fallback = ['window','self','document','navigator','location','history','console','alert','prompt','confirm','setTimeout','setInterval','clearTimeout','clearInterval','requestAnimationFrame','cancelAnimationFrame','fetch','XMLHttpRequest','localStorage','sessionStorage','indexedDB','crypto','performance','screen','top','parent','frames','length','name','closed','event','onload','onerror','onunload','origin','isSecureContext'];
      fallback.forEach(k => defaultKeys.add(k));
    }
